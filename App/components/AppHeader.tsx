...
import { Sparkles, PanelLeft, PanelLeftOpen, Cpu, ChevronDown, Phone, Plus, Edit3, Highlighter, Trash2, Check, Minus, Camera, Image, Moon, Sun, HelpCircle, Settings, BookMarked, CheckSquare, Bell } from 'lucide-react';
  ...
  return (
    <header className="h-[65px] flex-none border-b border-[rgba(0,0,0,0.15)] dark:border-[rgba(255,255,255,0.05)] flex items-center justify-between px-3 md:px-6 bg-white dark:bg-[#1a1a1a] min-w-0 gap-2 relative z-40">
      ...
      <div className="flex items-center gap-1 md:gap-2 flex-shrink-0">
        ...
        {props.onOpenNotebook && props.onTabChange && (
          <>
            <button 
              onClick={() => props.onTabChange!('notebook')} 
              className={`p-1.5 md:p-2 rounded-full transition-colors flex-shrink-0 relative ${
                props.activeTab === 'notebook' 
                  ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600'
                  : 'text-[#a0a0a0] hover:bg-[rgba(0,0,0,0.03)] dark:hover:bg-[#2a2a2a] hover:text-yellow-600'
              }`}
              title="Notebook"
            >
              <BookMarked size={16} />
            ...
            <button 
              onClick={() => props.onTabChange!('todos')} 
              className={`p-1.5 md:p-2 rounded-full transition-colors flex-shrink-0 relative ${
                props.activeTab === 'todos' 
                  ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600'
                  : 'text-[#a0a0a0] hover:bg-[rgba(0,0,0,0.03)] dark:hover:bg-[#2a2a2a] hover:text-blue-600'
              }`}
              title="Tasks"
            >
              <CheckSquare size={16} />
            ...
            <button 
              onClick={() => props.onTabChange!('reminders')} 
              className={`p-1.5 md:p-2 rounded-full transition-colors flex-shrink-0 relative ${
                props.activeTab === 'reminders' 
                  ? 'bg-red-100 dark:bg-red-900/30 text-red-600'
                  : 'text-[#a0a0a0] hover:bg-[rgba(0,0,0,0.03)] dark:hover:bg-[#2a2a2a] hover:text-red-600'
              }`}
              title="Reminders"
            >
              <Bell size={16} />
              {props.remindersCount && props.remindersCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-600 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">
                  {props.remindersCount}
                </span>
              )}
            </button>
          </>
