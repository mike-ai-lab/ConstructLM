...
/**
 * Citation Auto-Highlight Integration Test
 * Tests the COMPLETE workflow: File upload → AI response → Citation parsing → Highlighting
 * 
 * Run with: node test-citation-integration.js
 * 
 * Requirements:
 * - GROQ_API_KEY or CEREBRAS_API_KEY in environment
...
// Test configuration
const config = {
  apiKey: process.env.GROQ_API_KEY || process.env.CEREBRAS_API_KEY,
  provider: process.env.GROQ_API_KEY ? 'groq' : 'cerebras',
  model: process.env.GROQ_API_KEY ? 'llama-3.3-70b-versatile' : 'llama3.1-8b'
};
...
function testApiKey() {
  ...
  assert(
    !!config.apiKey && config.apiKey !== 'your_api_key_here',
