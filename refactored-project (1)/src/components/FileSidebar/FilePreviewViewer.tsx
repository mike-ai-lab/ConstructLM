...
import DocumentViewer from '../../components/DocumentViewer'; // Adjusted import based on context
...
const FilePreviewViewer: React.FC<FilePreviewViewerProps> = ({ file, onClose }) => {
  ...
  // Fix: Memoize image URL to prevent memory leaks
  const imageUrl = useMemo(() => {
    if (file.type === 'image' && file.fileHandle) {
      return URL.createObjectURL(file.fileHandle as File);
    }
    return null;
  }, [file]);

  // Fix: Revoke URL on cleanup
  useEffect(() => {
    return () => {
