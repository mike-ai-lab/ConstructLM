...
// Helper to flatten visible items for shift-select logic
export const getVisibleFileIds = (
  ...
  if (viewTab === 'folders') {
    ...
    const traverse = (nodes: Record<string, TreeNode>) => {
      ...
      for (const node of sortedNodes) {
        if (node.type === 'file' && node.file) {
          ...
          // We don't select folders themselves in this model, but we traverse if expanded
    ...
    // Helper to get files in a specific folder path
    const getFilesIn = (path: string) => files.filter(f => f.userFolder === path);

    rootFolders.forEach(folder => {
      if (expandedFolders.has(folder.path)) {
        const folderFiles = getFilesIn(folder.path);
        folderFiles.forEach(f => visibleIds.push(f.id));
      }
    });
