...
import { sendMessageToGemini, initializeGemini, cancelCurrentRequest } from './services/geminiService';
import { sendMessageToLLM } from './services/llmService';
import { MODEL_REGISTRY, DEFAULT_MODEL_ID } from './services/modelRegistry';
import { Send, Menu, Sparkles, X, FileText, Database, PanelLeft, PanelLeftOpen, Mic, Cpu, ChevronDown } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    const activeContextFiles = mentionedFiles.length > 0 ? mentionedFiles : files.slice(0, 3); // Max 3 files
    ...
    try {
      ...
      await sendMessageToLLM(
          ...
          activeContextFiles, 
          (chunk) => {
              // Handle both chunk-based (Groq) and full-text (Gemini) streaming
              if (typeof chunk === 'string' && chunk.length < accumText.length + 100) {
       ...
       const errorMsg = error?.message?.includes('overloaded') 
         ? "⚠️ Gemini is overloaded. Wait 30 seconds before trying again."
         : "❌ Request failed. Try with fewer files or shorter message.";
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full relative bg-white transition-all duration-300">
        ...
        <header className="h-14 flex-none border-b border-gray-100 flex items-center justify-between px-6 bg-white/80 backdrop-blur-sm z-10">
          ...
          <div className="text-xs text-gray-400 font-medium hidden sm:block">
              {files.length > 3 && <span className="text-orange-500">⚠️ Using first 3 files only</span>}
          </div>
        </header>
