...
const findAllFuzzyMatches = (fullText: string, quote: string): { start: number, end: number }[] => {
    ...
    if (cleanQuote.length < 3) return []; // Ignore very short quotes

    const matches: { start: number, end: number }[] = [];
    const escapeRegExp = (string: string) => string.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

    // Strategy 1: High Precision Regex with flexible whitespace
    // Handles newlines/tabs in source text vs spaces in quote
    try {
        const tokens = cleanQuote.split(/\s+/).filter(t => t.length > 0);
        if (tokens.length > 0) {
    ...
    } catch (e) { console.warn("Regex match failed", e); }
    
    if (matches.length > 0) return matches;

    // Strategy 2: Simple Literal Search (Case Insensitive)
    ...
    while (pos < lowerText.length) {
        ...
        pos = idx + 1; // Allow overlaps or move forward
    }
    if (matches.length > 0) return matches;

    // Strategy 3: Partial Match (First 50 chars) - helpful if quote is truncated/hallucinated at end
    if (cleanQuote.length > 20) {
        const partial = cleanQuote.substring(0, Math.min(50, cleanQuote.length));
        const partialLower = partial.toLowerCase();
        const idx = lowerText.indexOf(partialLower);
        if (idx !== -1) {
            matches.push({ start: idx, end: idx + partial.length });
        }
    }
...
const DocumentViewer: React.FC<DocumentViewerProps> = ({ file, initialPage = 1, highlightQuote, location, citations = [], onClose }) => {
  ...
  const renderHighlights = (textContent: any, viewport: any, quote: string): boolean => {
          ...
          itemMap.forEach(({ start, end, item }) => {
             if (Math.max(start, matchIndex) < Math.min(end, matchEnd)) {
                 try {
                     ...
                     const fontWidth = item.width * viewport.scale; // Visual width
                     const angle = Math.atan2(tx[1], tx[0]);

                     const rect = document.createElement('div');
                     rect.className = 'pdf-highlight-rect'; // Marker class
      ...
      // Fallback: Partial match if not found (First 30 chars)
      if (!found && normQuote.length > 20) {
          const partialQuote = normQuote.substring(0, 30);
          const partialIndex = fullText.indexOf(partialQuote);
          if (partialIndex !== -1) {
              // Just highlight the partial match as a "best effort"
              found = true;
              const matchEnd = partialIndex + partialQuote.length;
               itemMap.forEach(({ start, end, item }) => {
                    if (Math.max(start, partialIndex) < Math.min(end, matchEnd)) {
                        // ... same creation logic, abbreviated for brevity ...
                        try {
                            if (!window.pdfjsLib.Util) return;
                            const tx = window.pdfjsLib.Util.transform(viewport.transform, item.transform);
                            const fontHeight = Math.hypot(tx[2], tx[3]);
                            ...
                            Object.assign(rect.style, {
                                ...
                                width: `${Math.abs(fontWidth)}px`,
                                height: `${fontHeight}px`,
                                backgroundColor: 'rgba(255, 200, 0, 0.3)', // Slightly different color for partial
                                mixBlendMode: 'multiply',
                                pointerEvents: 'none',
                                transform: `rotate(${angle}rad)`,
                                transformOrigin: '0% 100%'
      ...
      // Store the scroll target on the container for the effect to pick up
      if (firstMatchElement && highlightLayerRef.current) {
  ...
  useLayoutEffect(() => {
    ...
    const performScroll = () => {
        ...
        if (excelRow) {
            ...
            setTimeout(() => {
             ...
             // If not found, check if we need to set notFound state
             // NOTE: Excel uses sync rendering, so if element is missing, it's missing.
             // Text uses fuzzy match logic. If no match elements, setNotFound is called below in renderTextContent logic implicitly? 
             // No, renderTextContent returns JSX. We can't know easily inside render.
             // Check if matches were found during render?
        }
    };
    ...
    requestAnimationFrame(() => {
        performScroll();
        // Double tap for dynamic content
        requestAnimationFrame(performScroll);
    });
    
  }, [highlightQuote, location, isPdf, file, textScale]); // textScale dependency ensures re-scroll on zoom
