...
// Chunk text with overlap - preserves page markers for PDFs
export function chunkText(text: string, chunkSize: number = 500, overlapPercent: number = 25): string[] {
  ...
  // Check if this is PDF content with page markers
  const pageRegex = /--- \[Page (\d+)\] ---([\s\S]*?)(?=--- \[Page \d+\] ---|$)/g;
  const hasPageMarkers = pageRegex.test(text);
  
  if (hasPageMarkers) {
    // PDF: chunk by page, preserving page numbers
    pageRegex.lastIndex = 0; // Reset regex
    let match;
    ...
    while ((match = pageRegex.exec(text)) !== null) {
      ...
      let currentChunk = `[Page ${pageNum}]\n`;
      
      for (const para of paragraphs) {
        if ((currentChunk + para).length <= chunkSize) {
          currentChunk += (currentChunk.endsWith('\n') ? '' : '\n\n') + para;
        } else {
          if (currentChunk.trim().length > `[Page ${pageNum}]`.length) {
            chunks.push(currentChunk.trim());
          }
          currentChunk = `[Page ${pageNum}]\n${para}`;
          
          // If single paragraph exceeds chunk size, split it
          while (currentChunk.length > chunkSize) {
            chunks.push(currentChunk.slice(0, chunkSize).trim());
            currentChunk = `[Page ${pageNum}]\n` + currentChunk.slice(step);
          }
        }
      }
    ...
    // Non-PDF: standard paragraph-based chunking
